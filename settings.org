...
При подключении учесть, что на больших файлах (10k+ строк) емакс начинает заикаться. Проблема есть как минимум в cperl-mode.
...
(use-package proof-general
  :config
  (setq coq-prog-name "~/.opam/4.05.0/bin/coqtop -emacs"))
#+END_SRC
