...
Этот конфиг использую на всех десктопах и телефонах. Особенности:
 - Из него динамически подгружаются файлы:
   * ~/.emacs.d/local/user-info.org : настройки email, имени пользователя и т.д.
   * ~/.emacs.d/local/org-agenda.org : личные настройки шаблон
   * ~/.emacs.d/local/org-capture.org
   * ~/.emacs.d/local/gnus-accounts.org
   * ~/.emacs.d/local/gnus-templates.org
...
(my-load-org-config "path/to/this/settings.org")
#+END_EXAMPLE
