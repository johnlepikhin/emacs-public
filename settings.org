...
Добавляем дополнительные пути в начало PATH и синхронизируем exec-path, чтобы пользовательские скрипты, утилиты и Guix-пакеты были доступны для вызовов из Emacs:

#+BEGIN_SRC emacs-lisp
(let ((extra-paths (list (expand-file-name "~/bin")
                         (expand-file-name "~/.guix-home/profile/bin"))))
  (setenv "PATH" (concat (string-join extra-paths ":") ":" (getenv "PATH")))
...
*** Timestamps в *Messages*

Добавляем временную метку к каждому сообщению в буфере =*Messages*= для удобства отладки:

#+BEGIN_SRC emacs-lisp
(defun my/message-with-timestamp (orig-fun fmt-string &rest args)
  "Добавляет timestamp к сообщениям в *Messages*."
  (when fmt-string
    (apply orig-fun
           (concat (format-time-string "[%F %T] ") fmt-string)
           args)))
...
*** Уведомления (dunstify)

 Универсальная обёртка для отправки уведомлений через dunstify.

 #+BEGIN_SRC emacs-lisp
 (defun my-dunstify (summary &optional body &rest args)
   "Показать уведомление через dunstify.
 SUMMARY — заголовок, BODY — текст (необязательно).
 ARGS — plist: :app :icon :urgency :timeout :replace-id."
   (when (executable-find "dunstify")
     (let ((cmd-args nil))
       (push "-a" cmd-args)
       (push (or (plist-get args :app) "Emacs") cmd-args)
       (when-let ((icon (plist-get args :icon)))
         (push "-i" cmd-args) (push icon cmd-args))
       (when-let ((urgency (plist-get args :urgency)))
         (push "-u" cmd-args) (push (number-to-string urgency) cmd-args))
       (when-let ((timeout (plist-get args :timeout)))
         (push "-t" cmd-args) (push (number-to-string timeout) cmd-args))
       (when-let ((rid (plist-get args :replace-id)))
         (push "-r" cmd-args) (push (number-to-string rid) cmd-args))
       (setq cmd-args (nreverse cmd-args))
       (nconc cmd-args (list summary))
       (when body (nconc cmd-args (list body)))
       (with-temp-buffer
         (let ((exit-code (apply #'call-process "dunstify" nil t nil cmd-args)))
           (unless (zerop exit-code)
             (message "dunstify exited with %d: %s"
...
** Gnus
 ...
 (defun my-gnus-zap-to-signature ()
   "Удалить текст от курсора до сигнатуры (-- )."
   (interactive)
   (let ((start (point)))
 ...
 Проверить почту через mbsync:

 #+BEGIN_SRC emacs-lisp
 (defun my-gnus-update-news-external ()
   "Синхронизировать почту через mbsync и обновить индекс notmuch."
   (interactive)
   (message "Syncing mail via mbsync...")
   (call-process-shell-command "(mbsync -a; notmuch new) &>/dev/null &" nil 0)
...
Перенаправление метаданных nnmaildir в кеш, чтобы не мешать mbsync:

#+BEGIN_SRC emacs-lisp
(require 'xdg)

(defvar my-nnmaildir-cache-base
  (file-name-as-directory (expand-file-name "gnus-nnmaildir" (xdg-cache-home)))
  "Каталог для метаданных nnmaildir вне Maildir (чтобы не мешать mbsync).")

(defvar my-nnmaildir-mail-base
  (file-name-as-directory (expand-file-name "~/.mail"))
  "Корень Maildir, синхронизируемый через mbsync.")

(defun my-nnmaildir-rewrite-path (path)
  "Перенаправить путь .nnmaildir в кеш-директорию.
Возвращает новый путь или nil, если перенаправление не нужно."
  (when (and (string-match-p "\\.nnmaildir" path)
             (string-prefix-p my-nnmaildir-mail-base path))
    (let ((relative (substring path (length my-nnmaildir-mail-base))))
      (concat my-nnmaildir-cache-base
              (replace-regexp-in-string "/?\\.nnmaildir/?" "/" relative)))))

(defun my-nnmaildir-file-handler (operation &rest args)
  "Прозрачно перенаправить файловые операции с .nnmaildir в кеш."
  (let ((inhibit-file-name-handlers
         (cons 'my-nnmaildir-file-handler
               (and (eq inhibit-file-name-operation operation)
                    inhibit-file-name-handlers)))
        (inhibit-file-name-operation operation))
    (let* ((new-args (mapcar (lambda (arg)
                               (if (stringp arg)
                                   (or (my-nnmaildir-rewrite-path arg) arg)
                                 arg))
                             args))
           (rewritten (and (stringp (car new-args))
                           (not (equal (car new-args) (car args)))
                           (car new-args))))
      ;; Обеспечить существование родительской директории в кеше
      (when rewritten
        (let ((parent (file-name-directory (directory-file-name rewritten))))
          (when (and parent (not (file-exists-p parent)))
            (make-directory parent t))))
      (apply operation new-args))))

(push '("\\.nnmaildir" . my-nnmaildir-file-handler)
      file-name-handler-alist)
#+END_SRC

Улучшение рендеринга HTML-писем — org-like стилизация поверх shr:
...
Подсветка синтаксиса в =<pre>= блоках HTML-писем:
...
#+END_SRC
 ...
 (use-package gnus
   ...
   :config
   ;; Улучшения рендеринга HTML-писем
   ...
   ;; gnus-topic-mode определена в gnus-topic.el, не в gnus.el —
   ;; нельзя ставить в :hook, иначе autoload не найдёт функцию
   (require 'gnus-topic)
   ...
   ;; Уведомления о новой почте (только новые письма, не все непрочитанные)
   (defvar my-gnus--unread-before nil
     "Alist счётчиков непрочитанных писем до обновления.")

   (defun my-gnus-save-unread-counts ()
     "Сохранить текущие счётчики непрочитанных перед обновлением."
     (setq my-gnus--unread-before
           (mapcar (lambda (info)
                     (let ((group (car info))
                           (unread (gnus-group-unread (car info))))
                       (cons group (if (numberp unread) unread 0))))
                   gnus-newsrc-alist)))

   (defun my-gnus-notify-new-mail ()
     "Уведомить о новых письмах через dunstify."
     (when my-gnus--unread-before
      (let ((new-count 0))
       (dolist (info gnus-newsrc-alist)
         (let* ((group (car info))
                (unread (gnus-group-unread group))
                (before (or (cdr (assoc group my-gnus--unread-before)) 0)))
           (when (and (numberp unread) (> unread before))
             (cl-incf new-count (- unread before)))))
       (when (> new-count 0)
         (my-dunstify (format "Новая почта: %d" new-count) nil
                      :app "Gnus" :icon "mail-unread" :urgency 1)))))

   (add-hook 'gnus-get-new-news-hook #'my-gnus-save-unread-counts)
   (add-hook 'gnus-after-getting-new-news-hook #'my-gnus-notify-new-mail))
 #+END_SRC
